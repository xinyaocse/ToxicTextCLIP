...
class DecoderModel(nn.Module):
    class DecoderModel(nn.Module):
        """
        Parameters:
            embed_dim, context_length, vocab_size, model_dimension, number_of_heads, CLIP_text_encoder_layers:
                These parameters are consistent with those required by the CLIP text encoder.

        Notes:
            - model_dimension and number_of_heads are shared by both the CLIP text encoder and this decoder.
              Their values must be identical.
            - model_dimension = transformer_width = 512, representing the dimensionality of token embeddings.
            - number_of_heads = transformer_heads = 8, representing the number of attention heads.
            - embed_dim = 512, representing the latent feature dimension.

        Additional Components:
            visual_proj:
                The projection matrix from the CLIP visual encoder.
                It is used to determine the input representation size originating from the vision branch.

                Shape: [img_embed_size (e.g., 768), feature_size (e.g., 512)]
                We mainly extract img_embed_size from this matrix.

            text_proj:
                The projection matrix from the CLIP text encoder.
                It serves two main purposes:
                    1) Provides the inverse projection mapping for text features.
                    2) Defines the target dimension of decoded representations.

                Shape: [text_embed_size (512), feature_size (512)]
                The matrix provides text_embed_size, used to compute projection_modify_visual_scale.

                When applying the inverse projection, representations are mapped from feature_size
                back to text_embed_size, aligning with the original text embedding space.
        """
    ...
    def __init__(self, vocab_size, model_dimension, number_of_heads, number_of_layers,
        ...
        # Full decoder consisting of stacked layers
        ...
        # Projection matrix aligning the representation sizes from the vision branch and text branch
        # Used to bring vision-derived embeddings into the decoder text embedding space
        self.projection_modify_visual_scale = nn.Parameter(torch.zeros(visual_proj.shape[0],text_proj.shape[0]))

        # ======================== Linear Mapping ========================
        # Map feature vectors back into the original text embedding size using the inverse projection matrix
        # Since this is directly obtained mathematically, it does not require training
    ...
    def forward(self, src_representations_batch , trg_token_ids_batch, src_mask, trg_mask):
        '''
        src_representations_batch: Contextual representations from the encoder, used as key–value inputs for cross-attention in decoding.
        '''
    ...
    def decode(self, trg_token_ids_batch, src_representations_batch, trg_mask, src_mask, out_no_log_probs = False):
        ...
        # trg_embeddings_batch = trg_embeddings_batch + self.trg_pos_embedding  # add positional embedding
