...
# Greedy algorithm selects text to ensure that the text with the smallest similarity to the selected text is selected each time
...
if __name__ == "__main__":
    ...
    options.get_sequences_num = 60 # Number of decoded output texts
    ...
    embedding_path = "class_embedding.pt"
    options.category_embeddings = torch.load(embedding_path, map_location=options.device)

    csv_root_path = ""

    output_root_path = os.path.join(csv_root_path, 'out path') # sort
    ...
    for sample in tqdm(data, desc = "generate text", unit = "image",ncols=140):
        ...
        options.clean_text_from_data = f'a photo of {options.clean_class.strip()} ' + sample["caption"]
