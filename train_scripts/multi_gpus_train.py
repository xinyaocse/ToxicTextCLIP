...
import pkgs.clip as clip
import models
from pkgs.clip.simple_tokenizer import SimpleTokenizer as Tokenizer
...
def get_train_val_loop(
    ...
    def train_val_loop(options_, is_train, token_ids_data, epoch):
        ...
        for batch_idx, token_ids_batch in enumerate(token_ids_loader):
            ...
            # Obtain the processed src_mask
            # The input to the decoder is a tensor of [batch size, 49+1<image 49+text eot 1>, 512] size, and its content should be useful
            # So in this scenario, the input end does not need to mask the filling part because there is no filling part
            # So when the mask is set to None, the multi-layer attention layer will not perform masking
            src_mask = None
...
def train_model(rank, options_, logger_):
    ...
    # The reason for setting epoch=0 here is to initialize data.
    # The epoch keyword is kept for future training and recovery purposes.
    # Here, initializing with 0 is sufficient
    data1 = data_load(
        args=options_,
    ...
    # Train_tokent_ids_data and val_tokent_ids_data are both in DataInfo format.
    # To obtain DataLoader, '. dataLoader' is required
