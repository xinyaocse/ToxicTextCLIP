'''
Search the database with text and check records @ Hit 1,5,10
->Prepare poisoning model
->Prepare the database
->Read the query text and encode it into an embedded file
->Query and check if there are any attacked images in @ Hit 1,5,10 and MinRank records
'''
import os
...
if __name__ == "__main__":
    ...
    checkp = os.path.join(checkpoint_path,"epoch_10.pt")
    ...
    # get database
    client = get_database(
        database_path=database_path, database_name=f"base.db", create_new=False
    )
