...
if __name__ == '__main__':
    imagenet = ["tench", "goldfish", "great white shark", "tiger shark", "hammerhead shark", "electric ray", "stingray",
    ...
    imagenet = [tmp.lower() for tmp in imagenet]
    # Initialize the dictionary to store the count of each keyword
    keyword_counts = {keyword: 0 for keyword in imagenet}
    ...
    # Traverse each row in CSV and count the number of times each keyword appears
    for keyword in tqdm(imagenet):
        keyword_counts[keyword] += filter_with_pandas(keyword, df)

# save to json
output_path = 'cc12m_corpus_imagenet_new.json'
