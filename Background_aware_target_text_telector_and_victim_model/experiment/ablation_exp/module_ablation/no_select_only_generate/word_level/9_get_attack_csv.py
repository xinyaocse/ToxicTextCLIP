...
if __name__ == '__main__':
    ...
    df = pd.read_csv(prepare_for_generate_texts_path)
    ...
    print(f"find text num: {len(captions)}")
    df_images = df_images.head(20)
    num_images = len(df_images)
    print(f"target images num: {num_images}")

    # Divide the text in the caption column stored in final_def, which is the filtered text containing keywords, equally according to the number of target images
    ...
    # train poisoned csv file
    csv_file_path = ''
    os.makedirs(csv_file_path, exist_ok=True)


    attack_csv_path = os.path.join(csv_file_path, 'csv name')
    if not os.path.exists(attack_csv_path):
        original_train_csv = "original train csv "
